...
void RGWGetObj::execute()
{
  ...
  if (s->merge_ref.bi_entry.meta.is_merged)
  {
    ...
    ldout(s->cct, 0) << "redirect read result:" << r << "," << bl_data.length() << ", " << (get_data ? "true" : "false") << dendl;
    if (r < 0)
    {
    ...
    else
    {
      ...
      do
      {
        ...
        ret = send_response_data(bl_data, sent_size, send_step);
        if (ret < 0) 
        {
        ...
        ldout(s->cct, 0) << "send data success:"
                         << " offset=" << sent_size
                         << " send_size=" << send_step
                         << dendl;        
      ...
      ldout(s->cct, 0) << "send response success" << dendl;
    }
    ret = send_response_data(bl_data, 0, 0); 
    if (ret < 0) 
      send_response_data(bl_data, 0, 0);    
    return;
  }
...
void RGWDeleteObj::execute()
{
  ...
  if (!s->object.empty()) {
    ...
    if (s->merge_ref.bi_entry.meta.is_merged)
    {
      ...
      ldout(s->cct, 0) << "delete merged object success" << dendl;
      return;
    }
    /*End added*/
...
/*Begin added by lujiafu*/      
    else if (-ENOENT== ret)
    {
      ...
      {
        ldout(s->cct, 0) << "read_permissions on " << s->bucket << ":" << s->object << ", obj is merged to:" 
                         << s->merge_ref.bi_entry.meta.data_pool << "/" 
                         << s->merge_ref.bi_entry.meta.data_oid << ", offset=" 
