...
{
  if (NULL != m_bgt_worker && RGW_BGT_WORKER_ACTIVE == m_bgt_worker->m_state)
  {
    ...
    ldout(cct, 0) << "oid=" << ref.oid << " key=" << ref.key << dendl;
    ldout(cct, 0) << "bucket id=" << bucket.bucket_id << " marker=" << bucket.marker << dendl;
    ...
    strcpy(log_entry.bucket, bucket.name.c_str());
    strcpy(log_entry.oid, key.name.c_str());
    strcpy(log_entry.bi_oid, bs.bucket_obj.c_str());
    ...
    {
      ldout(cct, 0) << "write change log success:" 
                    << "bucket=" << bucket.name 
                    << " oid=" << key.name 
...
int RGWRados::init_bgt_watcher(string& name, int role, RGWInstanceObjWatcher** watcher)
{
  ...
  if (RGW_ROLE_BGT_SCHEDULER == role)
  {
    instobj_watcher = new RGWBgtScheduler(instobj, this, cct);
  }
  else if (RGW_ROLE_BGT_WORKER == role)
  {
...
void RGWRados::get_obj_aio_completion_cb(completion_t c, void *arg)
{
  ...
  if (typeid(*(d->client_cb)) == typeid(RGWArchiveOp_CB))
  {
    ...
    {
      ldout(cct, 0) << " === op callback , task complete====" << dendl;    
                                             ...
                                             bool is_head_obj, void *arg)
{
  ...
  if(d->client_cb->get_op_type() == 1 ) {
    if (is_head_obj) {
      ...
      ldout(cct,0) << "read oid " << oid << dendl;
      /* only when reading from the head object do we need to do the atomic test */
      r = append_atomic_test(rctx, obj, op, &astate);
      if (r < 0)
        return r;
